...
type ResponseType = Id<"channels"> | null; // Corrected type

type Options = {
    onSuccess?: (data: Id<"channels"> |null) => void; // Fix: Corrected return type
...
export const useRemoveChannel = () => {
    ...
    const mutate = useCallback(async (value: RequestType, options?: Options) => {
        try {
            ...
            options?.onSuccess?.(response); // Corrected type
            return response;
        } catch (error) {
